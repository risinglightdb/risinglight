...
impl Binder {
    ...
    pub fn bind_expr(&mut self, expr: Expr) -> Result {
        let id = match expr {
            ...
            _ => {
                return Err(
                    ErrorKind::Todo(format!("bind expression: {:?}", expr)).with_spanned(&expr)
                )
            }
        }?;
        self.type_(id)?;
    ...
    fn bind_binary_op(&mut self, left: Expr, op: BinaryOperator, right: Expr) -> Result {
        ...
        let node = match op {
            ...
            Or => Node::Or([l, r]),
            Xor => Node::Xor([l, r]),
            _ => return Err(ErrorKind::Todo(format!("bind binary op: {:?}", op)).into()),
        };
    ...
    fn bind_unary_op(&mut self, op: UnaryOperator, expr: Expr) -> Result {
        ...
        Ok(match op {
            ...
            _ => return Err(ErrorKind::Todo(format!("bind unary operator: {:?}", op)).into()),
        })
    }
    ...
    fn bind_typed_string(&mut self, data_type: DataType, value: String) -> Result {
        match data_type {
            ...
            _ => return Err(ErrorKind::Todo(format!("typed string: {:?}", data_type)).into()),
        }
    }
    ...
    fn bind_interval(&mut self, interval: parser::Interval) -> Result {
        ...
        let value = DataValue::Interval(match interval.leading_field {
            ...
            f => return Err(ErrorKind::Todo(format!("interval with leading field: {f:?}")).into()),
        });
    ...
    fn bind_in_subquery(&mut self, expr: Expr, subquery: Query, negated: bool) -> Result {
        ...
        let &[col0] = schema.as_slice() else {
            return Err(ErrorKind::SubqueryMustHaveOneColumn(schema.len()).with_span(subquery_span));
        };
    ...
    fn bind_any_all_op(
        ...
        let Expr::Subquery(subquery) = right else {
            return Err(ErrorKind::InvalidAnyAllSubquery.with_spanned(&right));
        };
        let subquery_span = subquery.span();
    ...
    fn bind_function(&mut self, func: Function) -> Result {
        ...
        for arg in function_args {
            ...
            match arg {
                ...
                FunctionArgExpr::QualifiedWildcard(_) => {
                    return Err(
                        ErrorKind::Todo("qualified wildcard like `t.*`".into()).with_spanned(arg)
                    );
                }
            }
        }
        ...
        let node = match func.name.to_string().to_lowercase().as_str() {
            ...
            name => return Err(ErrorKind::NoFunction(name.to_string()).with_spanned(&func.name)),
        };
    ...
    fn bind_window_function(&mut self, func: Id, window: WindowType, name: &ObjectName) -> Result {
        ...
        if window.window_frame.is_some() {
            return Err(ErrorKind::Todo("window frame".into()).with_spanned(name));
        }
