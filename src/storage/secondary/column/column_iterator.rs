...
impl ColumnIteratorImpl {
    ...
    pub async fn next_batch(&mut self, expected_size: Option<usize>, filter_bitmap: Option<&BitVec>) -> Option<(u32, ArrayImpl)> {
        match self {
            Self::Int32(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
            Self::Float64(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
            Self::Bool(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
            Self::Char(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
            Self::Decimal(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
            Self::Date(it) => Self::erase_concrete_type(it.next_batch(expected_size, filter_bitmap).await),
        }
    }
