...
impl UnaryExecutor {
    ...
    pub fn eval_batch_standard<I1, O, F>(i1: &ArrayImpl, f: F) -> Result<ArrayImpl, FunctionError>
    where
        I1: ArrayValidExt,
        ...
        F: for<'a> Fn(&'a I1::Item) -> <O::Item as ToOwned>::Owned,
    {
        ...
        if zeros == 0 {
            ...
            // auto vectoried
            let res_iter = i1a_raw_iter.map(f);

            return Ok(O::from_data(res_iter, masks).into());
        } else if zeros == masks.len() {
            // all invalid
            ...
            for y in mask_chunks.by_ref() {
                ...
                if zeros == 0 {
                    ...
                    buffer
                        .iter_mut()
                        .enumerate()
                        .for_each(|(i, value)| *value = f(i1a.get_unchecked(base + i)));
                    ...
                    buffer.iter_mut().enumerate().for_each(|(i, value)| unsafe {
                        if *masks.get_unchecked(base + i) {
                            *value = f(i1a.get_unchecked(base + i));
                            res_count += 1;
                        }
                    });
            ...
            buffer
                ...
                .for_each(|(i, (value, mask))| {
                    if *mask {
                        *value = f(i1a.get_unchecked(cnt + i));
                        res_count += 1;
                    }
                });
            builder.extend_from_raw_data(&buffer[0..res_count]);
    ...
    pub fn eval_batch_lazy_select<I1, O, F>(l: &ArrayImpl, f: F) -> Result<ArrayImpl, FunctionError>
    where
        I1: ArrayValidExt,
