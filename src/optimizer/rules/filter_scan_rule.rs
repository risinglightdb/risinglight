use super::*;
use crate::optimizer::plan_nodes::LogicalFilter;
use crate::optimizer::plan_nodes::LogicalSeqScan;

pub struct FilterScanRule {}

impl Rule for FilterScanRule {
    fn apply(&self, plan: PlanRef) -> Result<PlanRef, ()> {
        let filter = plan.downcast_rc::<LogicalFilter>().map_err(|_| ())?;
        let scan = filter
            .child
            .clone()
            .downcast_rc::<LogicalSeqScan>()
            .map_err(|_| ())?;
        Ok(Rc::new(LogicalSeqScan {
            table_ref_id: scan.table_ref_id.clone(),
            column_ids: scan.column_ids.clone(),
            column_descs: scan.column_descs.clone(),
            with_row_handler: scan.with_row_handler,
            is_sorted: scan.is_sorted,
            expr: Some(filter.expr.clone()),
        }))
    }
}
