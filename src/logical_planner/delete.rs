...
impl LogicalPlaner {
    pub fn plan_delete(&self, stmt: BoundDelete) -> Result<Plan, LogicalPlanError> {
        if let BoundTableRef::BaseTableRef { ref ref_id, .. } = stmt.from_table {
            if let Some(expr) = stmt.where_clause {
                ...
                Ok(Plan::LogicalDelete(LogicalDelete {
                    table_ref_id: *ref_id,
                    child: Plan::LogicalFilter(LogicalFilter { expr, child }).into(),
                }))
            } else {
